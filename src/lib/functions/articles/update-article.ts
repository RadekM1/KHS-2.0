...
export const handleArticleChange = async (
    ...
    if (allInGallery && !thumbnail) {
      const newThumbnail = allInGallery[0].file;
      checkedThumbnail = newThumbnail.includes(
        "https://storage.googleapis.com/",
      )
        ? newThumbnail
        : `https://storage.googleapis.com/khs-zlin/img-gallery/${slug}/${newThumbnail}`;
    }
    ...
    if (allInGallery && allInGallery.length > 30) {
      ...
      const tempThumbnail = thumbnail ?? 'https://storage.googleapis.com/khs-zlin/logo.svg'
      ...
      if (readyToUploadFiles.length > 0) {
        const googleResponse = await handleGoogleUpload(editedArticleSlug, readyToUploadFiles);
        if (!googleResponse) {
