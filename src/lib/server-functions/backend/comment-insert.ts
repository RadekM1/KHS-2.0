...
export const commentInsert = async (
  slug: string,
  user: string,
  comment: string,
) => {
  ...
  try {
    if (!slug || !user || !comment) {
      ...
      return {
        ok: false,
        message:
          "chybí jedna z hodnot potřebných pro uložení komentáře (obsah komentáře, id uživatele, nebo id článku",
      };
    }
    ...
    if (!(response.rowCount > 0)) {
      ...
      return {
        ok: false,
        message:
          "komentář se nepodařilo uložit, zkuste znovu, případně kontaktujte podporu",
      };
    }
