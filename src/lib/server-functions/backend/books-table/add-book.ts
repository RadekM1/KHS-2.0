...
export const bookInsert = async (
  ...
  try {
    ...
    if (isImage) {
      ...
      returnedId = idResponse.rows[0].returnedid
        ? idResponse.rows[0].returnedid + 1
        : 1;

      if (!Number.isInteger(returnedId)) {
        return {
          ok: false,
          message: "Vrácená hodnota z databáze není celé číslo (integer)",
        };
      }
    }

    const bookImgUrl = isImage
      ? `https://storage.googleapis.com/khs-zlin/books/${returnedId}.jpg`
      : "";
