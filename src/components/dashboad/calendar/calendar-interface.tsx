      ...
      <div className="flex flex-col justify-center">
        ...
        <div className="flex flex-col justify-evenly md:flex-row">
          <ul className="px-5 flex-col m-2 py-3 my-6 border-gray-300 rounded-xl border-[1px] flex">
            <h3 className="text-gray-600 mb-2 dark:text-gray-300">
              Vybrané datum
            </h3>
            {date.map((event, i) => {
              const day = dayjs(event);
              const stringDate = day.format("DD.MM.YYYY");
              return <li key={i}>{stringDate}</li>;
            })}
          </ul>
          <div>
          ...
          {sortedRows.map((row) => {
            ...
            return (
              ...
              >
                ...
                <td>
                  <button disabled={loading} onClick={() => handleDel(row.id)}>
                    {loading ? (
                      ""
                    ) : (
                ...
                <td>
                  <button disabled={loading} onClick={() => handleEdit(row.id)}>
                    {loading ? (
                      ""
                    ) : (
